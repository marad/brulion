...
describe("versioned extension API contract", () => {
  it("describes the complete v1 public surface", () => {
    ...
    expect(contract.types.find((type) => type.name === "EditorSelection")?.declaration).toContain("head: number")
    expect(contract.types.find((type) => type.name === "EditorSelection")?.declaration).not.toContain("from:")
    expect(contract.types.find((type) => type.name === "EditorSelection")?.declaration).not.toContain("to:")
    const promptType = contract.types.find((type) => type.name === "PromptOptions")
    expect(promptType?.declaration).toContain("confirmLabel: string")
    expect(promptType?.declaration).not.toContain("okLabel")
    expect(declarations).toContain("confirmLabel: string")
    expect(declarations).not.toContain("interface PromptOptions {\n  okLabel")
