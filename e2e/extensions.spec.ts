...
test("runs an enabled local command and keeps management separate from the workbench", async ({ page }) => {
  ...
  await expect(workbench.locator("#workbench-tabs .workbench-tab")).toHaveCount(1)
  ...
  await workbench.locator("#workbench-new-tab").click()
  await expect(workbench.locator("#workbench-tabs .workbench-tab")).toHaveCount(2)
  await expect(workbench.locator("#workbench-file-title")).toHaveText("Select a file")
  ...
  await workbench.getByRole("button", { name: "Close helper.js" }).click()
